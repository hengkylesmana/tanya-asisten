...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // === AWAL PERUBAHAN: Elemen DOM Baru untuk Upload Gambar ===
    ...
    // === AWAL PERUBAHAN: State Baru untuk Gambar ===
    ...
    function init() {
        ...
        // === AWAL PERUBAHAN: Event Listener untuk Upload Gambar ===
    ...
    function initializeApp(mode = {}) {
        ...
        if (mode.isQolbu) {
            ...
            welcomeMessage = "Assalamualaikum, Bosku. Saya Asisten Qolbu siap membantu. Anda bisa bertanya tentang rujukan islami atau mengirim gambar untuk didiskusikan.";
        } else if (mode.isTest) {
            ...
            welcomeMessage = "Selamat datang, Bosku. Saya Dokter AI RASA. Ada keluhan medis yang bisa saya bantu? Anda bisa mengirim foto keluhan Anda jika perlu.";
        } else {
            headerTitle.textContent = "Asisten Pribadi";
            headerSubtitle.textContent = "Siap melayani, Bosku";
            welcomeMessage = "Selamat datang, Bosku. Saya, asisten pribadi Anda, siap mendengarkan. Ada yang bisa saya bantu? Anda juga bisa mengirim gambar.";
        }
    ...
    // === AWAL PERUBAHAN: Fungsi-fungsi Baru untuk Mengelola Gambar ===
    function handleImageSelection(event) {
        ...
        if (!file.type.startsWith('image/')) {
            displayMessage("Maaf, hanya file gambar yang didukung.", 'ai-system');
            return;
        }
        ...
        reader.onerror = () => {
            displayMessage("Gagal membaca file gambar.", 'ai-system');
        };
    ...
    function displayImagePreview(imageBase64) {
        ...
        imagePreviewContainer.appendChild(thumbnail);
    ...
    async function handleSendMessage() {
        ...
        // === AWAL PERUBAHAN: Kirim pesan jika ada teks ATAU gambar ===
        if (!userText && !selectedImageBase64) return;
        
        handleSendMessageWithText(userText, selectedImageBase64);
        
        userInput.value = '';
        userInput.style.height = 'auto';
        
        // Reset gambar setelah dikirim
        selectedImageBase64 = null;
        imagePreviewContainer.style.display = 'none';
        imagePreviewContainer.innerHTML = '';
        imageInput.value = '';
    ...
    async function handleSendMessageWithText(text, imageBase64 = null) {
        ...
        // === AWAL PERUBAHAN: Menambahkan gambar ke riwayat & tampilan ===
        const userMessage = text || (imageBase64 ? "Tolong jelaskan gambar ini." : "");
        conversationHistory.push({ role: 'user', text: userMessage });
        displayMessage(userMessage, 'user', imageBase64);
        // === AKHIR PERUBAHAN ===

        updateButtonVisibility();
        await getAIResponse(userMessage, imageBase64);
    }
    
    // === AWAL PERUBAHAN: `getAIResponse` menerima parameter gambar ===
    async function getAIResponse(prompt, imageBase64 = null) {
        ...
        try {
            const payload = { 
                ...
                mode: currentMode,
                image: imageBase64 // Kirim gambar ke backend
            };
    ...
    function updateButtonVisibility() {
        ...
        const canShowUpload = !isTyping && !hasImage && !isInputDisabled && currentMode !== 'psychologist';
        ...
        sendBtn.style.display = (isTyping || hasImage) && !isInputDisabled ? 'flex' : 'none';
        voiceBtn.style.display = !isTyping && !hasImage && !isInputDisabled ? 'flex' : 'none';
    ...
    // === AWAL PERUBAHAN: `displayMessage` menerima parameter gambar ===
    function displayMessage(message, sender, imageBase64 = null) {
        ...
        if (buttons.length > 0) {
            ...
            // Tambahkan container tombol ke dalam elemen teks jika ada, atau langsung ke messageElement
            const textDiv = messageElement.querySelector('div');
            if(textDiv) {
                textDiv.appendChild(choiceContainer);
            } else {
